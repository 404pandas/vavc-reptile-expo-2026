...
const INFO_PAGES = [
  {
    ...
    subtitle: 'Essential for aquatic turtles, frogs, and any reptile with standing water.',
    applies: ['Red-Eared Slider','Box Turtle','Dart Frog','Argentine Horned Frog','Red-Eyed Tree Frog','White Tree Frog','Chinese Water Dragon'],
    appliesSlugs: ['red-eared-slider','box-turtle','dart-frog','argentine-horned-frog','red-eyed-tree-frog','white-tree-frog','chinese-water-dragon'],
  },
  {
    title: 'UV Light',
    slug: 'uv-light',
    ...
    applies: ['Bearded Dragon','Tegu','Iguana','Russian Tortoise','Sulcata Tortoise','Uromastyx','Savannah Monitor'],
    appliesSlugs: ['bearded-dragon','tegu','iguana','russian-tortoise','sulcata-tortoise','uromastyx','savannah-monitor'],
  },
  {
    ...
    subtitle: 'How to safely prepare your reptile or tortoise for winter dormancy.',
    applies: ['Russian Tortoise','Box Turtle','Tegu','Bearded Dragon','Corn Snake','Ball Python','Uromastyx'],
    appliesSlugs: ['russian-tortoise','box-turtle','tegu','bearded-dragon','corn-snake','ball-python','uromastyx'],
  },
];

function buildInfoPage(p) {
  const applyLinks = p.applies.map((name, i) =>
    `<li><a href="/${p.appliesSlugs[i]}">${name}</a></li>`
  ).join('');
...
<main>
  ...
  <div style="background:${p.bg};padding:3rem 0 2.5rem;text-align:center;">
    <div class="container">
      <span style="font-size:3.5rem;display:block;margin-bottom:.5rem;">${p.emoji}</span>
      <h1 style="color:#fff;font-size:clamp(2rem,5vw,3.5rem);margin:0;">${p.title}</h1>
      <p style="color:rgba(255,255,255,.7);margin:.75rem auto 0;max-width:540px;font-size:1rem;">${p.subtitle}</p>
    </div>
  </div>
    ...
    <div class="container">
      <div class="row">
        ...
        <div class="col s12 l8">
          <div class="care-card" style="padding:0;overflow:hidden;border-left-color:${p.color};">
            <div style="padding:1.25rem 1.5rem .75rem;background:#fff;">
              <h3 style="margin:0;"><i class="material-icons tiny" style="color:#e65100;vertical-align:middle;margin-right:.4rem;">picture_as_pdf</i>${p.title} Guide</h3>
              <p style="margin:.4rem 0 1rem;color:#616161;font-size:.88rem;">View the full guide below, or download to keep.</p>
              <div style="display:flex;gap:.75rem;flex-wrap:wrap;margin-bottom:1rem;">
                <a href="/pdfs/${p.pdf}" download class="btn waves-effect download-btn">
                  <i class="material-icons left">download</i>Download PDF
                </a>
                <a href="/pdfs/${p.pdf}" target="_blank" class="btn waves-effect view-btn">
        ...
        <div class="col s12 l4">
          ...
          <div class="care-card" style="border-left-color:${p.color};">
            <h3><i class="material-icons tiny" style="color:${p.color};">pets</i> Relevant Animals</h3>
          ...
          <div class="care-card" style="background:#e8f5e9;border-left-color:#2e7d32;">
            ...
            <ul style="padding-left:1rem;">
              ...
              <li><a href="/hibernation">❄️ Hibernation &amp; Brumation</a></li>
            </ul>
